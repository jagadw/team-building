...
import { getTeams, deleteTeam, updateTeam, createTeam, type Team, type Event } from "../../services/teamService";
					...
					<tbody>
						{teams.map((team, index) => (
							<tr key={team.id} className="border-t border-gray-200">
								...
								<td className="p-2">{team.participants.find((p) => p.is_leader)?.name ?? "No leader"}</td>
								<td className="p-2">{team.event?.name}</td>
								<td className="p-2 text-center space-x-2">
									<button className="text-blue-600 hover:underline" onClick={() => handleEdit(team)}>
										Edit
									</button>
									<button className="text-red-600 hover:underline" onClick={() => handleDelete(team.id)}>
										Delete
									</button>
								</td>
							</tr>
					...
					<div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
						...
						<div className="space-y-3">
							...
							<div>
								...
								<input
									type="text"
									value={editingTeam.participants.find((p) => p.is_leader)?.name ?? ""}
									readOnly
						...
						<div className="mt-4 flex justify-end space-x-2">
							<button onClick={() => setEditingTeam(null)} className="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">
								Cancel
							</button>
